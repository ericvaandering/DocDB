# Description: The instructions for the meeting organizer and calendar in DocDB. 
...
valid password who is organizing a meeting. Several people can collaborate to
organize a meeting, but where changes collide or appear to collide, only the
first is taken.
<p/>

The meeting organizer system provides the ability to set up meetings with
...
<a name="create"/>
<h1>B. Creating a New Meeting</h1>

The first step in creating a meeting is to set up the sessions in the meeting.
You begin this process by clicking on "Oragnize a new meeting" from the DocDB
modifications page. Meetings can be
created as a subtopic in the DocDB or, for smaller meetings (perhaps by small
...
a topic or not, all the functionality of the meeting organizer is still
available. A list of the topics which are designated as "meeting-like" are
shown; simply select one of these to create the subtopic.<p/>

The "Show All Talks" selection controls what the user sees when viewing a
meeting.  In the meeting view, either all the sessions for a meeting with all
their talks can be shown or just links to the
various sessions. This should probably be checked for meetings with just a few
dozen talks, but left unchecked for larger meetings.<p/>

Space is provided to provide both long and short descriptions of the meeting.
If the meeting is being created with a subtopic, both of these descriptions are
used. If a subtopic is not being created, only the long description is used. Also
provided are spaces to specify a location and URL for the conference or meeting
along with the start and end dates.<p/>

The boxes labeled Meeting Preamble and Epilogue provide a space for text which
will be placed at the top and bottom respectively of the display of the
meeting. A welcome message or instructions can be placed here.<p/>

Finally, the View and Modify selections are used to control which groups may
view and modify the agenda for the meetings. The documents (talks) themselves
rely on their own security, not this setting.<p/>

A nearly identical form is used for modifying meeting information.  

<a name="sessions"/>
<h1>C. Creating Sessions</h1>

On the same form used for creating or modifying a meeting, the moderator is able
to set up one or more sessions in the meeting. If there are not enough spaces
...
Once at least one session has been added to the meeting, talks can be associated
with the sessions.<p/>

<a name="talks"/>
<h1>D. Adding and Modifying Talks in a Session</h1>
...
on the "Modify Meeting" page. This allows the moderator to add as many talks or
...
<a name="basicinfo"/>
<h2>1. Entering basic talk information</h2>
...
clicking on the "Note" link in the meeting or session displays. 

<a name="order"/>
<h2>2. Ordering the talks</h2>
...
<a name="confirm"/>
<h2>3. Specifying, deleting, and confirming documents</h2>
...
database, just the entry for the meeting.)<p/>
...
<a name="hints"/>
<h2>4. Giving hints about the talks</h2>
...
<a name="matching"/>
<h1>E. How the DocDB matches your entries with documents</h1>
...
<a name="userentry"/>
<h2>1. User selects the talk</h2>

When a user follows the link at the top of a session or meeting display that says "Add a talk to this
meeting of conference," they will see a document entry form with one small addition: a menu to select
his or her talk from the list of talks for that meeting or session that have not yet been entered.
...
<a name="hinting"/>
<h2>2. DocDB selects based on hints</h2>
...
with modification times in a time window around the meeting dates are
considered. Second, if the meeting is associated with a topic (discussed
...
<ul>
<li>Whether the document is associated with the meeting topic</li>
...
Documents that are associated with the meeting topic receive a bonus over
...
<a name="confirm2"/>
<h2>3. Moderator corrects and/or confirms</h2>

Finally, a moderator can manually enter document numbers and check "Confirm" or
just check "Confirm" to verify DocDB's choices. When managing a meeting, it is
...
For very small meetings (just a few talks) moderators may wish to not use hints
