...
sub SetAuthorMode {
  ...
  if ($AuthorMode ne "list" && $AuthorMode ne "field" && $AuthorMode ne "active") {
    $AuthorMode = $Preferences{Authors}{Selector} || $AuthorMode = "list";
  }
}
...
sub SetTopicMode {
  ...
  if ($TopicMode ne "single" && $TopicMode ne "multi"  && $TopicMode ne "tree") {
    $TopicMode = $Preferences{Topics}{Selector} || "tree";
  }
}
