...
sub FetchSecurityGroupsForFSSO (%) {
  ...
  # And finally from their cert as well if we are not transferring and $Preferences{Security}{UseCertGroupsForSSO} is true.
  if (!$Preferences{Security}{TransferCertToSSO} && $Preferences{Security}{UseCertGroupsForSSO}) {
